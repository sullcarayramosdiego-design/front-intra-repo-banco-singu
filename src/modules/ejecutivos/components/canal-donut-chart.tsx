...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  const d = payload[0]?.payload as DesempenioCanal;
  return (
    <div className="bg-zinc-900/95 border border-zinc-700 rounded-xl p-3 shadow-2xl text-xs space-y-1">
      ...
      <div className="flex justify-between gap-4">
        ...
        <span className="text-emerald-300 font-bold">${(d.monto_total / 1_000_000).toFixed(2)}M</span>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="rounded-xl border border-zinc-200 dark:border-zinc-700/60 bg-white dark:bg-zinc-900/80 p-5 space-y-3">
      <div>
        ...
        <p className="text-xs text-zinc-500 dark:text-zinc-400">Participación de cada canal en el total de operaciones</p>
      </div>
